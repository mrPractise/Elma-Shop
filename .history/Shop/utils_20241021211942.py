...
def generate_order_pdf(order_details, items, order_number):
    try:
        ...
        # Check if order_details is a string or a dictionary
        if isinstance(order_details, str):
            for line in order_details.split('\n'):
                elements.append(Paragraph(line, normal_style))
            total_amount = "N/A"  # Set a default value if total_amount is not available
            ...
            raise ValueError("order_details must be either a string or a dictionary")
        ...
        total_paragraph = Paragraph(f"<b>Total Order Amount:</b> <span style='background-color: yellow;'>Ksh.{total_amount}</span>", normal_style)
