...
def generate_order_image(order_details, items):
    ...
    img_width = 800
    img_height = 1000 + (len(items) * 150)  # Adjust height based on number of items
    ...
    d.text((10, 10), order_details, fill=(0,0,0), font=font)
    
    # Add product images and details
    y_offset = 200  # Start below the order details
    for item in items:
        # Download and paste the product image
        response = requests.get(item['image_url'])
        product_img = Image.open(BytesIO(response.content))
        product_img.thumbnail((100, 100))  # Resize image to fit
        img.paste(product_img, (10, y_offset))
        
        # Write product details
        d.text((120, y_offset), f"Product: {item['product_name']}", fill=(0,0,0), font=font)
        d.text((120, y_offset + 20), f"Quantity: {item['quantity']}", fill=(0,0,0), font=font)
        d.text((120, y_offset + 40), f"Price: Ksh.{item['price']}", fill=(0,0,0), font=font)
        
        y_offset += 150  # Move to next product
...
def thank_you(request):
    ...
    image_filename = generate_order_image(order_details, pending_order['items'])
    
    # Get the full URL for the image
    image_url = request.build_absolute_uri(settings.MEDIA_URL + 'order_images/' + image_filename)

    # Prepare the WhatsApp message with the image link
    whatsapp_message = f"New order received. View order details: {image_url}"
