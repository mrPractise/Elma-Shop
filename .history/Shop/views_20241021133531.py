    ...
    try:
       ...
       # Generate the order PDF
        try:
            ...
            pdf_filename = f"error_{order_number}.pdf"
            pdf_relative_url = reverse('error_pdf', kwargs={'order_number': order_number})

        # Logging instead of print statements
        logger.info(f"PDF generated for order {order_number}: filename={pdf_filename}, relative_url={pdf_relative_url}")

        # Get the full URL for the PDF
        pdf_url = request.build_absolute_uri(pdf_relative_url)
        logger.info(f"Full PDF URL for order {order_number}: {pdf_url}")

        # Prepare the WhatsApp message with the PDF link
        whatsapp_message = f"New order placed: {order_number}. View order details: {pdf_url}"

        # Add PDF information to the context
        context = {
            'order': pending_order,
