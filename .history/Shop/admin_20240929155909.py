...
class PurchaseOrderAdmin(admin.ModelAdmin):
    list_display = ('order_number', 'name', 'total_amount', 'created_at', 'status', 'item_count')
    list_filter = ('status', 'created_at')
    search_fields = ('order_number', 'name')
    inlines = [PurchaseOrderItemInline]
    readonly_fields = ('order_number', 'name', 'address', 'total_amount', 'created_at', 'item_count', 'items_display')
    
    fieldsets = (
        (None, {
            'fields': ('order_number', 'name', 'address', 'total_amount', 'created_at', 'status', 'item_count', 'items_display')
        }),
    )
    ...
    def items_display(self, obj):
        items = obj.items.all()
        return "\n".join([f"{item.quantity} x {item.product.name} - Ksh.{item.price}" for item in items])
...
class CartAdmin(admin.ModelAdmin):
    ...
    def clear_old_carts(self, request, queryset):
        threshold = timezone.now() - timedelta(days=30)  # Adjust as needed
        old_carts = Cart.objects.filter(is_active=False, created_at__lt=threshold)
        count = old_carts.count()
        old_carts.delete()
        self.message_user(request, f"Successfully cleared {count} old carts.")
    clear_old_carts.short_description = "Clear old inactive carts"
