...
def thank_you(request):
    ...
    return render(request, 'thankyou.html', context)def thank_you(request):
        """
        Render the thank you page after a successful order placement.
        """
        pending_order = request.session.get('pending_order')
        ...
        # Ensure WhatsApp number is a string
        whatsapp_number = settings.WHATSAPP_NUMBER
        if isinstance(whatsapp_number, tuple):
            whatsapp_number = whatsapp_number[0]  # Use the first number if it's a tuple
