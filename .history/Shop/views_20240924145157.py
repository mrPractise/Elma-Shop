    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for item in cart_items:
                if item.product.price != item.price or item.total_price != item.product.price * item.quantity:
                    # Cart item prices don't match current product prices
                    return render(request, 'checkout.html', {
                        'form': form,
                        'cart_items': cart_items,
                        'subtotal': subtotal,
        ...
        form = ShippingForm()
        # If there's a default shipping location, you might set it here
        default_location = ShippingLocation.objects.filter(is_default=True).first()
        if default_location:
