            ...
            if form.is_valid():
                ...
                with transaction.atomic():
                    ...
                    request.session['pending_order'] = {
                        ...
                        # In the checkout view, update the items dictionary in request.session['pending_order']
                        'items': [{
                            'product_id': item.product.id,
                            'product_name': item.product.name,
                            'short_description': item.product.short_description,  # Add this line
                            'quantity': item.quantity,
...
def thank_you(request):
    try:
        ...
        # Humanize prices in items and add descriptions
        items_with_description = []
        for item in pending_order['items']:
            # Get the product to access its description
            try:
                product = Product.objects.get(id=item['product_id'])
                description = product.short_description or "No description available"
            except Product.DoesNotExist:
                description = "Product details not available"

            formatted_item = {
                'product_name': item['product_name'],
                'description': description,  # Add description to the item
                'quantity': item['quantity'],
                'price': format_price(item['price']),
                'total': format_price(Decimal(str(item['price']).replace(',', '')) * int(item['quantity']))
            }
            items_with_description.append(formatted_item)

        # Generate the order PDF with descriptions
        try:
            pdf_filename, pdf_relative_url = generate_order_pdf(order_details, items_with_description, order_number)
            if not pdf_filename or not pdf_relative_url:
                raise Exception("PDF generation failed")
        except Exception as e:
            logger.error(f"Error generating PDF for order {order_number}: {str(e)}")
            return redirect('error_order', order_number=order_number.replace('/', '_'))
...
def error_order(request, order_number):
    return render(request, 'order-error.html', {'order_number': order_number})
